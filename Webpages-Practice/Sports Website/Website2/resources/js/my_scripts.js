...
*/
 ...
 function viewStudentStats(id, toggle){
	...
	if(toggle == 0){ //hide the html tag, visibility set to hidden and height to 0
		document.getElementById(id).style.visibility = "hidden"; 
		document.getElementById(id).style.height = 0; 
	}
	else if(toggle == 1){ //make the html tag visible, visibility set to visible and height set to auto 
...
function loadStatsPage(){ 
	var table = document.getElementById("stats_table"); //retrieves our stats_table
	var row_count; //for row index
	var col_count; //for column index
	var cell_value;
	...
	for(row_count=2; row_count < table.rows.length; row_count++){//outer loop iterates over each row
		//console.log(table.rows[row_count].cells.length)
		for(col_count = 0; col_count < table.rows[row_count].cells.length; col_count++){
			
			if(col_count == 1){ //gets opposing team name
			oppTeam = table.rows[row_count].cells[col_count].innerHTML;
			//alert(oppTeam); 

			
			}
			else if(col_count == 2){ //gets home score
			homeScore = table.rows[row_count].cells[col_count].innerHTML;
			
			//alert(homeScore);
			}
			else if(col_count == 3){ //gets opposing team score
			oppScore = table.rows[row_count].cells[col_count].innerHTML;
		...
		lossCount++; 
		table.rows[row_count].cells[4].innerHTML = oppTeam;
		}
	}
	

	var wins = document.getElementById("wins");
	wins.innerHTML = winCount; //updates wins for the buffs

	var losses = document.getElementById("losses");
	losses.innerHTML = lossCount; //updates losses for the buffs
